    ...
    try {
      ...
      if (!req.user || !req.user._id) {
        logger.error(
          ...
          "Critical: req.user._id not found in createProfile. Auth middleware issue?"
        );
        return res
          .status(500)
      ...
      if (err.name === "ValidationError") {
        ...
        ); // Use req.user?._id for safety if req.user might be undefined
      ...
      if (err.code === 11000) {
        logger.warn(
          { err, profileData: req.body, userId: req.user?._id },
          "Duplicate bot profile name on create."
        );
        return res
          .status(409)
          .json({
  ...
  updateProfileById: async (req, res, next) => {
    try {
      ...
      if (!req.user || !req.user._id) {
        logger.error(
          { path: req.path, method: req.method },
          "Critical: req.user._id not found in updateProfileById. Auth middleware issue?"
        );
        return res
          .status(500)
          .json({ message: "User authentication data missing." });
  ...
  deleteProfileById: async (req, res, next) => {
    try {
      ...
      if (!req.user || !req.user._id) {
        logger.error(
          { path: req.path, method: req.method },
          "Critical: req.user._id not found in deleteProfileById. Auth middleware issue?"
        );
        return res
          .status(500)
          .json({ message: "User authentication data missing." });
